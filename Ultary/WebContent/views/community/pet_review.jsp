<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%
	ArrayList<Post> rlist = (ArrayList<Post>)request.getAttribute("rlist");
	...
	<div id="all">
		<div id="all-wrap">
			...
			<%@ include file ="/views/common/cm_aside.jsp" %>
				<section>
				<div class="maincap">
        			<p class="cap_p_head">펫리뷰</p><br>
        			<p class="cap_p_tag">반려동물 제품/병원/장소 등 후기를 공유할수있는 게시판입니다.</p>
        			<hr id="mainhr">
        		</div>
     	...
     	<div class="cmcontent">
     		<div class="cm_wrap">
	     		...
	     		<% if(!rlist.isEmpty()) { %>
	     			<!-- 맨 처음으로 가는 버튼 -->
	     				<button onclick="location.href='<%=request.getContextPath()%>/cmrlist.po?currentPage=1'">&lt;&lt;</button>
	     			<!-- 이전 페이지로  -->
				<button onclick="location.href='<%= request.getContextPath()%>/cmrlist.po?currentPage=<%= currentPage -1 %>'" id="beforeBtn">&lt;</button>
				<script>
					if(<%= currentPage %> <= 1){
			...
			<button id="choosen" disabled><%= p %></button>
				<% } else {%>
					<button id="numBtn" onclick="location.href='<%=request.getContextPath() %>/cmrlist.po?currentPage=<%= p %>'"><%= p %></button>
				<% } %>
			<% } %>
			<!-- 다음 페이지로 -->	
			<button id="afterBtn" onclick="location.href='<%= request.getContextPath()%>/cmrlist.po?currentPage=<%= currentPage+1 %>'"	>&gt;</button>
			<script>
			...
			<!-- 맨 끝으로 -->
			<button onclick="location.href='<%= request.getContextPath() %>/cmrlist.po?&currentPage=<%=maxPage %>'">&gt;&gt;</button>
		<% } %>
	     		</div>
	     	</div>
	     	</div>
		...
		<script>
				...
				btn.click(function(){
				 ...
				 $('.wbtn').click(function(){
			        	location.href="<%= request.getContextPath() %>/views/community/cmpostWrite.jsp?cnum=4";
			        });
		</script>
</body>
</html>